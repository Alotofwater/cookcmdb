...
# 自己
from mrest_api.serializers_list.redislist import RedisListModelSerializer
...
class RedislistView(ViewSetMixin, APIView):
        ...
        if request.version == 'v1':
            try:
                ...
                for rdsdict in postdata.get("postdata"): # type:dict
                    # 判断提交的key数据是否正常
                    keyjudge  = ['InstanceName','InstanceId','ConnectionDomain',
                    ...
                    rdsjudge = models.Redislist.objects.filter(InstanceId=InstanceIdjudge)
