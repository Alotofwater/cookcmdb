...
from utils.mtasks import aliyun_redislist
import logging
...
class Redislist_list_Config(StarkConfig):
    ...
    def display_applicationlist_unit(self, row=None, header=False):
        if header:
            return "关联应用名"
        applicationlist_unit = row.applicationlist_unit.all()
        class_name_list = [ "%s" %(row.title) for row in applicationlist_unit]
    ...
    list_display = [
                    ...
                    'Engine',
                    'EngineVersion',
                    display_applicationlist_unit,
                    display_create_at_date, # 创建时间
                    display_latest_date_date, # 修改时间
    ]
    # 搜索条件
    search_list = ["title","InstanceName",'InstanceId']
    ...
    # 自定义操作
    def multi_collect(self, request):
        """
        采集阿里云的redis相关配置
        :param request:
        :return:
        """
        aliyun_redislist.delay()
        url = reverse('stark:%s' %(self.get_list_url_name))
        return redirect(url)

    multi_collect.text = "采集redis信息"


    action_list = [StarkConfig.multi_delete,multi_collect]